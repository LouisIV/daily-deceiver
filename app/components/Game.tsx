...
import { getLayout, shuffle, selectRoundsWithMinReals } from "@/lib/game/snippets";
...
export function Game({ totalQuestions }: Props) {
  ...
  const loadGame = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const selected = selectRoundsWithMinReals(daily.snippets, TOTAL, getLayout);
      setRounds(selected);
      setCurrent(0);
      setScore(0);
      setAnswered(null);
  ...
  const next = useCallback(() => {
    if (current + 1 >= rounds.length) {
      setPhase("over");
      const [gradeTitle] = getGrade(score, rounds.length);
      ...
      <div style={{ maxWidth: 780, margin: "0 auto", padding: "0 16px 80px" }}>
        ...
        {phase === "over" && (
          <ResultScreen
            score={score}
            total={rounds.length}
            grade={getGrade(score, rounds.length)}
            history={history}
            restart={loadGame}
          />
        )}
