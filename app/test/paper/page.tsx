...
function VariantCard({
  ...
  return (
    ...
    >
      <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--aged)", background: "var(--paper)" }}>
        <div style={{ fontWeight: 700, color: "var(--ink)", marginBottom: 4 }}>{title}</div>
        <div style={{ fontSize: 13, color: "var(--rule)" }}>{description}</div>
      </div>
      <div style={{ position: "relative", minHeight: 160, background: "var(--paper)" }}>
        <div
          style={{
            position: "absolute",
            inset: 0,
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: "0 auto", fontFamily: "system-ui, sans-serif" }}>
      ...
      <p style={{ color: "var(--rule)", marginBottom: 24, fontSize: 15 }}>
        Capability: filter renders = {filterRenders === null ? "…" : filterRenders ? "yes" : "no"}
      </p>
